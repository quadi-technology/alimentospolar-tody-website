...
class GW_Go_Portfolio {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( $this->plugin_slug . '-script', self::$plugin_prefix . '_settings', array( 
				...
				'showsocial' => isset( $general_settings['showsocial'] ) ? 'true': 'false',
				'socialposition' => isset( $general_settings['socialposition'] ) ? $general_settings['socialposition'] : 'position:absolute;top:100%;right:0;left:0;',
				'socialpositionsmallscreen' => isset( $general_settings['socialpositionsmallscreen'] ) ? $general_settings['socialpositionsmallscreen']: 'position:absolute;top:100%;right:0;left:0;',
				'socialdirection'=> isset( $general_settings['socialdirection'] ) ?  $general_settings['socialdirection'] : 'horizontal',
				'socialrotateeffect' => isset( $general_settings['socialrotateeffect'] ) ? 'true': 'false',
				'showfacebook' => isset( $general_settings['showfacebook'] ) ? 'true': 'false',
				'showtwitter' => isset( $general_settings['showtwitter'] ) ? 'true': 'false',
				'showdownload' => isset( $general_settings['showdownload'] ) ? 'true': 'false',
				'vid_plugin_url' => plugins_url('viddownload.php',__FILE__)
			) 
		);  
	}
	/**
							...
							if ( isset( $post_types[$portfolio['post-type']] ) ) {
								...
								<div id="<?php echo esc_attr( self::$plugin_prefix . '_' . $portfolio['id'] ); ?>" style="<?php echo esc_attr( ( isset( $margin_top ) ? 'margin-top:' . $margin_top . ';' : '' ) . ( isset( $margin_bottom ) ? 'margin-bottom:' . $margin_bottom . ';' : '' ) ); ?>">
										...
										<div class="gw-gopf-posts-wrap-inner">
											<div class="gw-gopf-posts gw-gopf-clearfix"<?php echo ( isset( $portfolio['column-layout'] ) && !empty( $portfolio['column-layout'] ) ? ' data-col="' . preg_replace('/[^0-9]/', '', $portfolio['column-layout'] ) . '"' : '' ); ?><?php echo ( $layout_type == 'slider' ? ' data-slider="' . esc_js( json_encode( $slider_data ) ) . '"' : '' ); ?>>
												...
												<div id="<?php echo esc_attr( $post->ID ); ?>" class="gw-gopf-col-wrap" data-filter="<?php echo esc_attr( implode(' ', $post_term_list ) ); ?>">
													...
													$post_lb_button_data_raw = '<a data-video-id="'.$post_meta['gw_go_portfolio_lightbox_youtube_video_id'][0].'" data-mfp-type="'.$post_meta['gw_go_portfolio_lightbox_video_type'][0].'" title="' . esc_attr( isset( $portfolio['lightbox-caption'] ) ? trim( get_the_title() ) : '' ) . '" data-id="' . $post->ID . '_' . $portfolio_key . '" href="' . $lighbox_link . '" data-content="' . $lighbox_content . '" data-mfp-src="' . $lighbox_link . '" class="' . $lighbox_class . ' gw-gopf-post-overlay gw-gopf-post-overlay-link"' . ( isset( $popup_height ) ? ' data-height="' . $popup_height . '"' : '' ) . '>';
													         $post_lb_button = '<a data-video-id="'.$post_meta['gw_go_portfolio_lightbox_youtube_video_id'][0].'" data-mfp-type="'.$post_meta['gw_go_portfolio_lightbox_video_type'][0].'" title="' . esc_attr( isset( $portfolio['lightbox-caption'] ) ? trim( get_the_title() ) : '' ) . '" data-id="' . $post->ID . '_' . $portfolio_key . '" href="' . $lighbox_link . '" data-content="' . $lighbox_content . '" data-mfp-src="' . $lighbox_link . '" class="' . $button_style_class . ' ' . $lighbox_class . '"' . ( isset( $popup_height ) ? ' data-height="' . $popup_height . '"' : '' ) . '>' . $button_content . '</a>';
									...
									if ( isset( $portfolio['pagination-type'] ) && $portfolio['pagination-type'] == 'load-more' ) {
										$load_more_button_text = isset( $portfolio['load-more-button-text'] ) && !empty( $portfolio['load-more-button-text'] ) ? $portfolio['load-more-button-text'] : 'Cargar Más';
										$load_more_button_loading_text = isset( $portfolio['load-more-button-loading-text'] ) && !empty( $portfolio['load-more-button-loading-text'] ) ? $portfolio['load-more-button-loading-text'] : '...';
										$button_src = '<a href="#" class="gw-gopf-pagination-load-more' . esc_attr( isset( $portfolio['load-more-button-style'] ) && !empty( $portfolio['load-more-button-style'] ) ? ' ' . $portfolio['load-more-button-style'] : '' ) .  '" data-original="Cargar Más" data-modified="Cargando">' . "Cargar Más" . '</a>';
									}
									?>
