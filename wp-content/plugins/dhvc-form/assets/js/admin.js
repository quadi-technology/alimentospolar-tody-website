...
(function ($) {
	...
	var recipients_email_type = function(select){
		if(select.is(':hidden'))
			return;
		
		var recipients_email_field = $('div.notice_recipients_field');
		var recipients_variables_field = $('p.notice_recipients_variables_field');
		recipients_email_field.hide();
		recipients_variables_field.hide();
		if(select.val() == 'email_text'){
			recipients_email_field.show();
		}else if(select.val() == 'email_field'){
			recipients_variables_field.show();
		}
	}
	recipients_email_type($('select#recipients_email_type'));
	$(document).on('change','select#recipients_email_type',function(){
		recipients_email_type($(this));
	})
	
	var setting_notice = function(checkbox){
		var notice_arr  = ['notice_recipients_variables_field','notice_name_field','notice_email_type_field','notice_variables_field','notice_email_field','notice_recipients_field','notice_reply_to_field','notice_subject_field','notice_body_field','notice_html_field'];
