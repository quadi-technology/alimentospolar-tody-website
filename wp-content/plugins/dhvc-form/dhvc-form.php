...
if(!defined('DHVC_FORM_VERSION'))
	define('DHVC_FORM_VERSION','1.4.25');
...
class DHVCForm {
	...
	public function init(){
		...
		/* add action for call check uniqe value validation */
		add_action('wp_ajax_dhvc_form_unique', array($this,'check_unique'));
		add_action('wp_ajax_nopriv_dhvc_form_unique', array($this,'check_unique'));
		
		
		add_action('wp_ajax_dhvc_form_unique_with_status', array($this,'check_unique_with_status'));
		add_action('wp_ajax_nopriv_dhvc_form_unique_with_status', array($this,'check_unique_with_status'));
...
#dhvcform-'.$post->ID.'.dhvc-form-flat .dhvc-form-textarea textarea{
	...
	#dhvcform-'.get_the_ID().'.dhvc-form-flat .dhvc-form-textarea textarea{
		...
		$dhvcformL10n = array(
			...
			//'unique_ajax_verify'=>apply_filters('dhvc_form_unique_use_ajax_verify', false) ? 'yes':'no',
	...
	/* uniqe validation check funciton */
	...
	public function check_unique_with_status(){ 
		global $dhvcform_db;
		if( !check_ajax_referer('dhvc_form_ajax_nonce', '_ajax_nonce', false) ) {
			echo 0;
			die;
		...
		foreach ($entries as $i=>$entry){
			$entry_data = (array) maybe_unserialize($entry->entry_data);
			if(array_key_exists('status', $entry_data)){
				if($entry_data['status'] == 'active' || $entry_data['status'] == 'waiting'){
					$unique_entry[] = $entry_data[$unique_key];	
				}
			}else{
				$unique_entry[] = $entry_data[$unique_key];
	...
	private function _form_handler($form_id, $form_controls, $save_entry = true,$send_notice = true, $autoreply = true,$return=false){
		...
		$posted_data['entrie_id'] = md5($last_inserted_id);
		...
		if($notice && $send_notice){
			...
			$mailer_to = array();
			$recipients_email_type = dhvc_get_post_meta($form_id,'_recipients_email_type','email_text');
			if($recipients_email_type == 'email_field'){
				$recipients_variables = dhvc_get_post_meta($form_id,'_notice_recipients_variables');
				if($recipients_variables){
					if(isset($posted_data[$recipients_variables]) && dhvc_form_validate_email($posted_data[$recipients_variables])){
						$mailer_to[] = trim((string)$posted_data[$recipients_variables]);
					}
				}
			}else{
	...
	protected function _radiobtn_redirect($data){
		if(isset($data) && $data['mayor'] == "0"){			
			wp_redirect( get_permalink('629') );exit;
		}else if(isset($data) && $data['mayor'] == "1"){
			wp_redirect( get_permalink('655') );exit;
		}
	}
