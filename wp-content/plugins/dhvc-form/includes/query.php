...
class DHVCFormQuery {
	...
	public function get_last_insert_entry_data(){
		global $wpdb;
		$last_insertd_id = "";
		$sql = "";
		...
		if(count($result) > 0){
			$last_insertd_id = $result[0]->id;	
		}
		return $last_insertd_id;
	}
	...
	public function get_entries($form_id = 0,$orderby='submitted',$order='desc',$limit = 10,$offset=0){
		...
		$sql .= "SELECT * FROM `" . $this->get_form_entry_data_table_name() . "` `entries` ";
		if(is_array($form_id)){
			$form_id_count = count($form_id);
			$counter = 0;
			$sql .= "WHERE (";
			foreach ( $form_id as $key => $value) {
				$sql .= "`entries`.`form_id` = $value ";
				$counter++;
				if($counter < $form_id_count){
					$sql .= " OR ";	
				}
			}
			$sql .= ") ";
...
global $dhvcform_db;
$dhvcform_db = DHVCFormQuery::instance();
